...
const EditVehiclePage = () => {
  const { register, control, handleSubmit, formState: { errors }, reset } = useForm<IVehicle>();
  ...
  const onSubmit = async (data: Partial<IVehicle>) => {
    try {
        await api.put(`/vehicles/${id}`, {...data, isFavorite: false})
        navigate('/');
    } catch(err) {
        console.log(err);
    }
    ...
    <div className={styles.Vehicles}>
      <main className={styles.main}>
        ...
        <div className="row d-flex flex-row align-items-center mb-4">
          ...
          <div className="col">
            <h1 className="mb-0">Edit Vehicle ({id})</h1>
          </div>
        </div>
